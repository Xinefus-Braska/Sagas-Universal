...
Please see the combat_handler script file for more details.
"""
import random
...
def resolve_grappling_action(character, action):
    ...
    Because mma is so fluid, the position the character and victim are currently
...
def apply_damage(attacker, victim, attack_type, critical_hit):
    """
    ...
    also do checks to determine if the victim's health, stamina, and
    ...
    if attack_type == "submission":
        ...
        damage = round(roll( (attacker.ability_scores.Str.actual * attacker.ndb.eq_damage / victim.ndb.eq_phy_arm), \
                 'very flat', attacker.ability_scores.Str))
        victim.traits.hp.current -= damage
        log_file(f"{attacker.name} hit {victim.name} for {damage} damage. \
                 They have {victim.traits.hp.actual} hps left.", \
                 filename='combat.log')
        if victim.traits.hp.actual < 1:
            # TODO: Implement death - for now we'll just flee
            victim.execute_cmd('flee')
        return damage


def check_shield_block_multiplier(victim):
    """
    Checks to see if a victim is currently equipping a shield and applies
    a blocking modifier based upon the armor value of the shield to the
    chance to block.
    """
    shield_block_multiplier = 1
    for slot in victim.db.slots:
        if slot != None:
